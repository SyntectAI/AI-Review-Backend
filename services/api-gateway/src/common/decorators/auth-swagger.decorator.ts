...
const AuthExamples = {
  ...
  responses: {
    ...
    existingUser: {
      summary: 'User with login already exists',
      value: {
        statusCode: 409,
...
export const AuthSwagger = {
  signUp: () => {
    const decorators = [
      ApiOperation({
        description:
          'Create a new user account with login and password. Returns a JWT access token upon successful registration.',
      }),
      ApiBody({
  ...
  signIn: () => {
    const decorators = [
      ApiOperation({
        description:
          'Authenticate user with login and password. Returns a JWT access token upon successful authentication.',
      }),
      ApiBody({
